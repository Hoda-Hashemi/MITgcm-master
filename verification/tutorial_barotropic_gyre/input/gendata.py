...
h.astype('>f4').tofile('bathy.bin')
#!WIND STRESS
tauMax = 0.0  # wind stress maximum
x = (np.arange(nx)-1) / (nx-2)   # x-coordinate, located at XG points
y = (np.arange(ny)-.5) / (ny-2)  # y-coordinate, located at YC points
Y, X = np.meshgrid(y, x, indexing='ij')
...
# # Grid dimensions from SIZE.h and script
# nx = 62
# ny = 62

# # File to read
# filename = 'windx_cosy.bin'  # Change to 'windx_cosy.bin' or 'windx_siny.bin' as needed

# # Read binary file (big-endian, 32-bit floats)
# data = np.fromfile(filename, dtype='>f4').reshape(ny, nx)

# # Print shape and sample values
# print(f"File: {filename}")
# print(f"Shape: {data.shape}")  # Should be (62, 62)
# print(f"Sample values (first 5x5):\n{data[:5, :5]}")

# # Visualize
# plt.imshow(data, origin='lower', cmap='viridis')
# plt.colorbar(label='Depth (m)' if 'bathy' in filename else 'Wind Stress (N/m²)')
# plt.title(f'Contents of {filename}')
